...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
int start(){int LotDigits;double Lots;double MinLot=MarketInfo(Symbol(),MODE_MINLOT);
if(MinLot==0.001)LotDigits=3;if(MinLot==0.01)LotDigits=2;if(MinLot==0.1)LotDigits=1;
double MMLot=NormalizeDouble(AccountBalance()*RiskFactor/100.00/100.00,LotDigits);
...
{OrderModify(OrderTicket(),OrderOpenPrice(),JawsNow,OrderTakeProfit(),0,CLR_NONE);}}}}return(0);}
//+------------------------------------------------------------------+
//|---------------------------// END //--------(24/08/2011)----------|
//|      Copyright © 2011 http://www.mql4.com/ru/users/rustein       |
//+------------------------------------------------------------------+