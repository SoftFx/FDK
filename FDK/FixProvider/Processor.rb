...
class Processor
  ...
  def populateField2TypeHash
    @field2TypeHash = Hash.new    
    @fields.elements.each("field") { |field|
      name = field.attributes["name"]
      type = field.attributes["type"]
      @field2TypeHash[name] = type;
    }
  end
  ...
  def lookupType( name )
    ...
    type = @field2TypeHash[name];
    raise "type of field '#{name}' not found" if type == nil
    if (("STRING" == type) || ("CURRENCY" == type) || ("DATA" == type) || ("LOCALMKTDATE" == type))
      return "std::string";
    elsif (("INT" == type) || ("NUMINGROUP" == type) || ("LENGTH" == type))
      return "int";
    elsif ("CHAR" == type)
      return "char";
    elsif (("FLOAT" == type) || ("QTY" == type) || ("PRICE" == type) || ("AMT" == type) || ("PERCENTAGE" == type))
      return "double";
    elsif ("UTCTIMESTAMP" == type)
      return "FIX::UtcTimeStamp";
    elsif ("UTCTIMEONLY" == type)
      return "FIX::UtcTimeOnly";
    elsif ("UTCDATEONLY" == type)
      return "FIX::UtcDate";
    elsif ("BOOLEAN" == type)
      return "bool";
    else
  ...
  def processField( element , generator)
    name = element.attributes["name"];
    type = lookupType(name);
    number = lookupField(name);
    generator.field(name, type, number); 
  end  
  ...
  def header
    ...
    @header.elements.each("field") { |element|
      @generators.each { |generator| processField( element, generator ) }
      }
  ...
  def component( element )
    ...
    component.elements.each("field") { |field|
      @generators.each { |generator| processField(field, generator)}
    }
