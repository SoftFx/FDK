...
void CLocalCppServer::SendMarketHistoryMetadataReport(void* handle, int64 id, const string& requestId, int32 status, const string& field)
{
	CServer* pServer = reinterpret_cast<CServer*>(handle);
	pServer->SendMarketHistoryMetadataReport(id, requestId, status, field);
}
