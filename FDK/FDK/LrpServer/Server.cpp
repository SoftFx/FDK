...
void CServer::SendMarketHistoryMetadataReport(int64 id, const string& requestId, int32 status, const string& field)
{
	CChannelSharedAccessor accessor(id, m_channels);
	CChannel* pChannel = accessor.GetChannel();
	if (nullptr != pChannel)
	{
		pChannel->Outgoing().SendDataHistoryMetaInformationFileResponse(requestId, status, field);
	}
}
