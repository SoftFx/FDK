...
T GetFieldValue(const FIX::FieldMap& fields)
{
	F field;
...
/// </summary>
template<typename F, typename T>
void SetFieldValue(const T& value, FIX::FieldMap& fields)
{
	F field;
...
/// </summary>
template<typename F, typename T>
bool TryGetFieldValue(const FIX::FieldMap& fields, T& value)
{
	F field;
	const bool result = fields.isSetField(field);
	if (result)
	{
		fields.getField(field);
...
#define FIELD_SET_EX( type, name ) \
inline type Get##name()const\
{\
	return GetFieldValue<FIX::name, type>(*this);\
}\
inline void Set##name(const type& value)\
{\
	SetFieldValue<FIX::name, type>(value, *this);\
}\
inline bool TryGet##name(type& value)const\
{\
	return TryGetFieldValue<FIX::name, type>(*this, value);\
}
