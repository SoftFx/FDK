...
namespace FDK
{
	...
	void CTradeEntry::Construct(TradeType type, TradeSide side, const char* symbol, double price, double volume, double commission, double agentCommission, double swap, Nullable<double> staticMarginRate)
	{
		m_type = type;
		m_side = side;
	...
	void CTradeEntry::SetPrice(const double newPrice)
	{
		m_price = ValidatePositiveOrZeroValue(__FUNCTION__, "newPrice", newPrice);
	}
	...
	TradeEntryStatus CTradeEntry::DoCalculateProfit(ptrdiff_t zzz, CRates& rates)
	{
		...
		{
			profit = (quote.Value().Bid - m_price) * GetNativeVolume();
		}
		else if (TradeSide_Sell == m_side)
		{
			profit = (m_price - quote.Value().Ask) * GetNativeVolume();
		}
		else
		{
