...
std::string string_strip( const std::string& value )
{
  ...
  int startPos = static_cast<int>(value.find_first_not_of(" \t\r\n"));
  int endPos = static_cast<int>(value.find_last_not_of(" \t\r\n"));
...
int socket_createAcceptor(int port, bool reuse)
{
  ...
  int socket = static_cast<int>(::socket( PF_INET, SOCK_STREAM, 0 , FX_SOCKET_MODE_SIMPLE ));
...
int socket_createConnector( int mode )
{
    return static_cast<int>(::socket(PF_INET, SOCK_STREAM, IPPROTO_TCP, mode));
...
int socket_accept( int s )
{
  ...
  return static_cast<int>(accept(s, 0, 0));
