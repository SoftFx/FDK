...
namespace FIX44
{
  ...
  public:
    ...
    QuoteRequest(
      ...
      const FIX::StreamReference& aStreamReference )
    : Message(MsgType())
    {
      set(aQuoteReqID);
      set(aStreamReference);
    }

    FIELD_SET(*this, FIX::QuoteReqID);
    FIELD_SET_EX(std::string, QuoteReqID);
    FIELD_SET(*this, FIX::StreamReference);
    FIELD_SET_EX(std::string, StreamReference);
    FIELD_SET(*this, FIX::RFQReqID);
    FIELD_SET_EX(std::string, RFQReqID);
    ...
    NoRelatedSym() : FIX::Group(146,55,FIX::message_order(55,65,48,22,454,460,461,167,762,200,541,224,225,239,226,227,228,255,543,470,471,472,240,202,947,206,231,223,207,106,348,349,107,350,351,691,667,875,876,864,873,874,913,914,915,918,788,916,917,919,898,711,140,303,537,336,625,229,54,854,38,152,516,468,469,63,64,193,192,15,232,1,660,581,555,735,692,40,62,126,60,218,220,221,222,662,663,699,761,423,44,640,235,236,701,696,697,698,453,0)) {}
      ...
      FIELD_SET_EX(int, QuoteRequestType);
      ...
      FIELD_SET_EX(int, QuotePriceType);
