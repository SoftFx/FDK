...
extern int Okrug = 2;  // Округление числа с плавающей запятой до указанной точности

extern int ProcentOtSvobDepo = 1; // процент риска от депозита

extern double MinLot = 0.10; // минимальный лот для торговли, если расчетный меньше минимального лота (зависит от счета)  

extern int MagicNumber = 888; // По этому номеру о4рентируется эксперт,
                              // каждый эксперт открывает, модифицирует, удаляет свои ордера со своим MagicNumber 
                              
extern double Porog = 0.00050;  // Порог не ближе которого можно выставить отложенный ордер

extern double SL = 0.00050; // StopLoss
extern double TP = 0.00100; // TakeProfit
extern double P  = 0.00020; // Количество пунктов на прорыв
...
//===================== Определения лота и риска ===================================

if ( Lot < MinLot )
 {
   Lot=MinLot;
 }
Comment( "\n","Допустимый риск ",ProcentOtSvobDepo, "% = ",Risk," от свободных средств ",SvobDepo," в лотах = ",Lot);
