//+------------------------------------------------------------------+
//|                                                   EnvelopeMA.mq4 |
//|                Andrй Duarte de Novais, Cassio Jandir Pagnoncelli |
//|                                            www.inf.ufpr.br/cjp07 |
//+------------------------------------------------------------------+
#property copyright "Andrй Duarte de Novais, Cassio Jandir Pagnoncelli"
#property link      "www.inf.ufpr.br/cjp07"

/*
   - Este й um SHORTER
   
   - Otimizaзхes: TimeCurrent() % (Period()*60) == 0
   
   - Otimizaзхes: Usar Fibonacci Levels (com seguranзa) para encerramento 
*/
...
int start()
{
   // Aguarda completar o candle para tomar decisхes
   ...
   // Administra S/L e T/P das posiзхes abertas
   int i;
   ...
   // Estratйgia de envelopes para short
   EnvelopeMMshort();
...
/*
   Estratйgia Envelope com fechamentos em MM
*/
void EnvelopeMMshort()
{
   ...
   // abre venda а descoberto pendente
...
double Lote() {
   ...
   // uso mбximo da margem de forma que um movimento de `ContraPips' pips tornem a conta sem liquidez (100% drawdown) 
   // respeitando um mнnimo associado com a alavancagem da conta (1:100=100pips, 1:200=50pips, 1:400=25pips)
   if ( ContraPips != 0 ) {
...
/*
   Calcula o preзo de fechamento p/ imediato (preзo de mercado) para ordem short/long
*/
...
/*
   Encerra todas as posiзхes abertas
*/
...
void ContaEmTela()
{
   Comment( "Lucro: ", AccountProfit(), 
            ...
            ", Liquidez/Balanзo: ", AccountEquity()/AccountBalance());
}
