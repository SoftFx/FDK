...
extern int gain = 2;          // nombre de fois de spread oщ il faudra clфturer
...
//----
   ...
   cntg = 0;               // nombre de position gagnйes
...
//----
   ...
   if (pos == false) {
      Print ("premier tick");
      debug ();
      retour = OrderSend (sym, OP_BUY, lot, Ask, 3, 0, 0, "J\'achиte !!!", magic, 0, Blue);
      if (retour < 1) {
            error = GetLastError();
   ...
   if (pos == true)  {
      ...
      Print ("Erreur programme; cette ligne n\'aurait pas due кtre exйcutйe!!!");
      debug ();
...
//+------------------------------------------------------------------+
//| fonction de vйrification de gains/pertes                         |
//+------------------------------------------------------------------+
void verif  () {
   int icon;
   string mess;
   ...
   if (cntp + cntg >= 5)   {
      if (cntp == cntg) {
         ...
         mess = "Les gains arrivent, continuez bien sыr?";
         icon = 64;
      }
      int button = MessageBox (mess, "Rйcapitulatif des gains/pertes",  MB_YESNO|icon|MB_TOPMOST);
      if (button == IDYES) {
         pos = 0;
         ordre = 0;
         return (0);
